...
Double_t myFunc(Double_t *x, Double_t *par)
{
  // par[0] = A, par[1] = tau
  Double_t xx = x[0];
  Double_t val = par[0] * TMath::Exp(-xx / par[1]);
  return val;
}
...
void myMacro()
{
  ...
  TH1F *stdHisto = new TH1F("stdHisto", "Rumore", 100, -5, 5);
  std::ifstream in;
  in.open("rumore.txt");
  Float_t rumore;
  while (1)
  ...
  TF1 *fitFunc = new TF1("fitFunc", myFunc, -5, 0, 2);
  fitFunc->SetParameters(1, 1); // settati a caso
