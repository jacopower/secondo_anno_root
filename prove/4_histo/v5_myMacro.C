...
void myMacro(Int_t nGen = 1000)
{
  ...
  h[0]->FillRandom("gaus", nGen); // predefined G(0,1)

  TF1 *f = new TF1("f", myFunction, -10, 10, 3); // user defined func
  f->SetParameter(0, 1);
  f->SetParameter(1, 0);
  f->SetParameter(2, 1);
...
// Per compilare da terminale ho bisogno di un main
